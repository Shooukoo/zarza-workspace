...
### 1. Secretos centralizados — ⬜ No iniciado
Rotación de `FCM_TOKEN_ENCRYPTION_KEY`/`INFERENCE_AUTH_TOKEN`, backfill de tokens FCM que quedaron en texto plano antes del cifrado, y mover `INTERNAL_NOTIFY_TOKEN` a un secret manager en vez de variable de entorno plana.
