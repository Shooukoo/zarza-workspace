...
export class PrismaAnalysisRepository implements IAnalysisRepository {
  ...
  async save(analysis: AnalysisDomain): Promise<string> {
    if (!analysis.campo_id) {
      throw new Error(
        `campo_id is required but was null — ensure campoId is sent in the upload request`,
      );
    }
    if (!analysis.productor_id) {
      throw new Error(
        `productor_id is required but was null — ensure the requester user exists in the database`,
      );
    }
    ...
    const record = await this.prisma.$transaction(async (tx) => {
      const created = await tx.analysis.create({
        data: {
          ...
          porcentajeMermaGeneral:
            analysis.metricas_salud.porcentaje_merma_general,
